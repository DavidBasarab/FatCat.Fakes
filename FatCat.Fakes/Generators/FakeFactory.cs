...
namespace FatCat.Fakes.Generators
{
    internal class FakeFactory
    {
        ...
        {
            if (TypeGenerators.ContainsKey(generatorType))
            {
                TypeGenerators[generatorType] = generator;
            }
            else
            {
                TypeGenerators.TryAdd(generatorType, generator);
            }
        }
        ...
        public object GetValue(Type type)
        {
            if (type.IsEnum)
            {
                return new EnumGenerator().Generate(type);
            }
